...
def build_nodes(width, height, matrix=None, density=None):
    """
    ...
    for y in range(height):
        ...
        for x in range(width):
            ...
            if matrix and matrix[y][x]:
                walkable = False
                pix = 999
            nodes[y][x] = Node(x, y, walkable, pix)
    return nodes


class Grid(object):
    def __init__(self, width=None, height=None, matrix=None, density=None):
        """
        ...
        self.nodes = build_nodes(self.width, self.height, matrix, density)
